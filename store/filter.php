...
<head>
    ...
    <title>Flat Finder(Filter)</title>
...
<?php
    $select = " WHERE ";
    if($_POST['type'] == "PrivateHall"){
        $_POST['type'] = "Private Hall";
    }
    if($_POST['type'] != "tAll"){
        if($_POST['period'] != "pAll"){
            $select = $select."RoomType = \"".$_POST['type']."\" AND Period = \"".$_POST['period']."\"";
        } else {
            $select = $select."RoomType = \"".$_POST['type']."\"";
        }
    } else{
        if($_POST["period"] != "pAll"){
            $select = $select."Period = \"".$_POST['period']."\"";
        } else {
            $select = "";
        }
    }

    if($_POST['price'] != "priall"){
        if($select == ""){
            $select = " WHERE FeePerW = ".$_POST['price'];
            switch ($_POST['price']) {
                case "100":
                    $select = " WHERE FeePerW < 100";
                    break;
                case "150":
                    $select = " WHERE FeePerW >= 100 AND FeePerW < 150";
                    break;
                case "200":
                    $select = " WHERE FeePerW >= 150 AND FeePerW < 200";
                    break;
                case "200+":
                    $select = " WHERE FeePerW >= 200";
                    break;
                default:
                    break;
            }
            ...
            switch ($_POST['price']) {
                case "100":
                    $select = $select." AND FeePerW < 100";
                    break;
                case "150":
                    $select = $select." AND FeePerW >= 100 AND FeePerW < 150";
                    break;
                case "200":
                    $select = $select." AND FeePerW >= 150 AND FeePerW < 200";
                    break;
                case "200+":
                    $select = $select." AND FeePerW >= 200";
                    break;
                default:
                    break;
            }
...
<div class="line"></div>
    <form method="POST" action="filter.php">
        ...
        <?php
            if($_POST['type'] == "tAll"){
                echo("<input type='radio' name='type' class='ButtonState' checked id='tAll' value='tAll'/>");
            } else {
                echo("<input type='radio' name='type' class='ButtonState' id='tAll' value='tAll'/>");
            }  
        ?>
        ...
        <?php
            if($_POST['type'] == "House"){
                echo("<input type='radio' name='type' class='ButtonState' checked id='House' value='House'/>");
            } else {
                echo("<input type='radio' name='type' class='ButtonState' id='House' value='House'/>");
            }
        ?>
        ...
        <?php
            if($_POST['type'] == "Flat"){
                echo("<input type='radio' name='type' class='ButtonState' checked id='Flat' value='Flat'/>");
            } else {
                echo("<input type='radio' name='type' class='ButtonState' id='Flat' value='Flat'/>");
            }
        ?>
        <label class="Button" for="Flat">Flat</label>
        
        <?php
            if($_POST['type'] == "Private Hall"){
                echo("<input type='radio' name='type' class='ButtonState' checked id='PrivateHall' value='PrivateHall'/>");
            } else {
                echo("<input type='radio' name='type' class='ButtonState' id='PrivateHall' value='PrivateHall'/>");
            }
        ?>
        ...
        <?php
            if($_POST['period'] == "pAll"){
                echo("<input type='radio' name='period' class='ButtonState' checked id='pAll' value='pAll'/>");
            } else {
                echo("<input type='radio' name='period' class='ButtonState' checked id='pAll' value='pAll'/>");
            }  
        ?>
        ...
        <?php
            if($_POST['period'] == "Long"){
                echo("<input type='radio' name='period' class='ButtonState' checked id='Long' value='Long'/>");
            } else {
                echo("<input type='radio' name='period' class='ButtonState' id='Long' value='Long'/>");
            }  
        ?>
        <label class="Button" for="Long">Long(> 6 months)</label>

        <?php
            if($_POST['period'] == "Short"){
                echo("<input type='radio' name='period' class='ButtonState' checked id='Short' value='Short'/>");
            } else {
                echo("<input type='radio' name='period' class='ButtonState' id='Short' value='Short'/>");
            }  
        ?>
        ...
        <?php
            if($_POST['price'] == "priAll"){
                echo("<input type='radio' name='price' class='ButtonState' checked id='priAll' value='priAll'/>");
            } else {
                echo("<input type='radio' name='price' class='ButtonState' id='priAll' value='priAll'/>");
            }  
        ?>
        ...
        <?php
            if($_POST['price'] == "100"){
                echo("<input type='radio' name='price' class='ButtonState' checked id='100' value='100'/>");
            } else {
                echo("<input type='radio' name='price' class='ButtonState' id='100' value='100'/>");
            }  
        ?>
        <label class="Button" for="150">0-100</label>

        <?php
            if($_POST['price'] == "150"){
                echo("<input type='radio' name='price' class='ButtonState' checked id='150' value='150'/>");
            } else {
                echo("<input type='radio' name='price' class='ButtonState' id='150' value='150'/>");
            }  
        ?>
        ...
        <?php
            if($_POST['price'] == "200"){
                echo("<input type='radio' name='price' class='ButtonState' checked id='200' value='200'/>");
            } else {
                echo("<input type='radio' name='price' class='ButtonState' id='200' value='200'/>");
            }  
        ?>
...
<div class="line"></div>
    ...
    <?php
        ...
        if (mysqli_num_rows($result) > 0){
            ...
            echo("<div style='position: relative; width: 85%; padding-left: 15%; font-size: 20px;'>");
            echo("Sorry~~ Currently Not Avaliable.");
            echo("</div>");
        }
        ...
        $rem = $total_num % 4;
        // echo("The value of check is ".$check.". The reminder is ".$rem);
        ...
        if ($num <= $total_num){
            ...
            // get the features
            echo("<div style='position: relative; width:100%;'>");
            echo("<table style='width:".($rem*17.5)."%; margin-left: 15%;'>");
                echo("<tr>");
                while($num <= $total_num){
                    $sql = "SELECT * FROM Store WHERE ID = ".$arr[$num-1];
